...
/**
 * Class representing an entry in a document-oriented database<br>
 * A document is split in two files: one for the data and one for the
 * indexes<br>
 * example for a document named toto:<br>
 * toto.dat:<br>
 * PapayaDB20161204<br>
 * toto.index:<br>
 * name=0 8<br>
 ...
 * month=12 2<br>
 * day=14 2<br>
 * Note there are two numbers for each index. The first one is the position of
 * the data and the second the length of the data.
 * 
 * @author jason
 *
 */
public class Document {
	...
	private Document(String filepath) throws IOException {
		...
		fileChannel = FileChannel.open(Paths.get(filepath + ".dat"), StandardOpenOption.CREATE, StandardOpenOption.READ,
				StandardOpenOption.WRITE);
	}

	private void fillDataArray(StringBuilder indexSb, StringBuilder dataSb, JsonArray arr, String s,
			Holder<Integer> index) {
		int i = 0;
		for (JsonValue v : arr) {
	...
	private void fillDataObject(StringBuilder indexSb, StringBuilder dataSb, JsonObject obj, String s,
			Holder<Integer> index) {
		if (obj == null) {
			return;
		}
	...
	/**
	 * Retrieves data of the document
	 * 
	 * @param where
	 *            A list of values to check
	 * @return The data of the document. If the <code>where</code> clause is not
	 *         met, a null value
	 * @throws IOException
	 *             If some I/O error occurs
	 * @throws IllegalStateException If the document has been closed
	 */
	public Map<String, String> select(String where) throws IOException {
		synchronized (fileChannel) {
			if (!fileChannel.isOpen()){
				throw new IllegalStateException("fileChannel is closed");
			}
			MappedByteBuffer map = fileChannel.map(MapMode.READ_ONLY, 0, fileSize);
			HashMap<String, String> results = new HashMap<>();
			String[] fields = where.split("&");
			for (String f : fields) {
				...
				if (toWrite == null && kv.length > 1 || toWrite != null && kv.length == 1) {
					return null;
				}
				switch (comparison) {
				case '=':
